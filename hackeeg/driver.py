...
GAINS = {1: ads1299.ADS1298_GAIN_1X,
         2: ads1299.ADS1298_GAIN_2X,
         3: ads1299.ADS1298_GAIN_3X,
         4: ads1299.ADS1298_GAIN_4X,
         6: ads1299.ADS1298_GAIN_6X,
         8: ads1299.ADS1298_GAIN_8X,
         12: ads1299.ADS1298_GAIN_12X}
...
class HackEEGBoard:
    ...
    def close(self):
        """
        This method closes the serial port connection.
        """
        if self.raw_serial_port is not None:
            self.raw_serial_port.close()
            self.raw_serial_port = None

    def _serial_write(self, command):
        self.serial_port.write(command)
        self.serial_port.flush()

    def _serial_readline(self, serial_port=None): # Al conectarse 1ra vez sale None
    ...
    def send_text_command(self, command):
        if self.debug:#<------EDITADO
            print(f"command: {command}  parameters: {None}")
